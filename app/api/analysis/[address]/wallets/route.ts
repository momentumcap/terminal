...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ address: string }> }) {
  ...
  return realtimeJson({ refreshSeconds: CACHE_TTLS.analysisMs / 1000, wallets: analysis.smartMoney.wallets, clusters: analysis.clusters, smartMoney: analysis.smartMoney });
}
